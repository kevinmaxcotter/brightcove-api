...
const REQUIRED_ENV = [ 'BRIGHTCOVE_ACCOUNT_ID', 'BRIGHTCOVE_CLIENT_ID', 'BRIGHTCOVE_CLIENT_SECRET', 'BRIGHTCOVE_PLAYER_ID', ]; const missingEnv = REQUIRED_ENV.filter(k => !process.env[k]); if (missingEnv.length) { console.error('Missing .env keys:', missingEnv.join(', ')); process.exit(1); }
...
// ---------------- Helpers ---------------- const looksLikeId = s => /\d{9,}$/.test(String(s).trim()); const splitTerms = input => String(input || '') .split(',') .map(s => s.trim().replace(/"(.)"$/, '$1').replace(/^'(.)'$/, '$1')) .filter(Boolean); const esc = s => String(s).replace(/"/g, '\"'); const escapeHtml = s => String(s).replace(/[&<>"']/g, m => ({ '&': '&', '<': '<', '>': '>', '"': '"', "'": ''' }[m])); const titleContainsAll = (video, terms) => { const name = (video.name || '').toLowerCase(); return terms.every(t => name.includes(t.toLowerCase())); }; const hasAllTags = (video, terms) => { const vt = (video.tags || []).map(t => String(t).toLowerCase()); return terms.every(t => vt.includes(t.toLowerCase())); }; const fmtDate = iso => { const d = new Date(iso); return isNaN(d) ? 'Unknown' : d.toISOString().slice(0, 10); };

// ---------------- Auth (token cache) ---------------- let tokenCache = { token: null, expiresAt: 0 }; async function getAccessToken() { const now = Date.now(); if (tokenCache.token && tokenCache.expiresAt > now) { return tokenCache.token; } const r = await axios.post( 'https://oauth.brightcove.com/v4/access_token', 'grant_type=client_credentials', { auth: { username: process.env.BRIGHTCOVE_CLIENT_ID, password: process.env.BRIGHTCOVE_CLIENT_SECRET }, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } } ); const token = r.data.access_token; const ttl = (r.data.expires_in || 300) - 30; // buffer tokenCache = { token, expiresAt: now + ttl * 1000 }; return token; }
...
; const fields = 'id,name,images,tags,state,created_at,published_at'; const r = await axios.get(url, { headers: { Authorization:
Bearer ${token}
}, params: { q, fields, sort, limit, offset } }); return Array.isArray(r.data) ? r.data : []; }

async function fetchAllPages(q, token) { const out = []; let offset = 0; const limit = 100; while (true) { const batch = await cmsSearch(q, token, { offset, limit }); out.push(...batch); if (batch.length < limit) break; offset += limit; if (out.length > 5000) break; // safety cap } return out; }
...
async function cmsRecentVideos(token, count = 20) { const url =
https://cms.api.brightcove.com/v1/accounts/${AID}/videos
; const fields = 'id,name,images,tags,state,created_at,published_at'; const params = { q: 'state:ACTIVE', fields, sort: '-created_at', limit: count }; const r = await axios.get(url, { headers: { Authorization:
Bearer ${token}
}, params }); const data = Array.isArray(r.data) ? r.data : []; return data .filter(v => v && v.state === 'ACTIVE') .map(v => ({ id: v.id, name: v.name || 'Untitled', tags: v.tags || [], created_at: v.created_at, published_at: v.published_at, thumb: (v.images && (v.images.thumbnail?.src || v.images.poster?.src)) || 'https://via.placeholder.com/320x180.png?text=No+Thumbnail' })); }
...
for (const id of idTerms) { try { const v = await fetchVideoById(id, token); if (v && v.state === 'ACTIVE') pool.push(v); } catch (e) { // ignore missing IDs } }
...
// ---------------- Analytics ---------------- async function getMetricsForVideo(videoId, token) { const alltimeViewsUrl =
...
const m = (metricsResp.data && metricsResp.data.items && metricsResp.data.items[0]) || {}; const alltimeViews = alltimeResp.data?.alltime_video_views ?? 0;
...
return { id: videoId, title, tags, views: alltimeViews, dailyAvgViews, impressions: m.video_impression || 0, engagement: m.engagement_score || 0, playRate: m.play_rate || 0, secondsViewed: m.video_seconds_viewed || 0, }; }

// ---------------- Concurrency helper ---------------- async function mapConcurrent(items, limit, mapper) { const results = new Array(items.length); let i = 0; const workers = new Array(Math.min(limit, items.length)).fill(0).map(async () => { while (i < items.length) { const idx = i++; try { results[idx] = await mapper(items[idx], idx); } catch (e) { results[idx] = e; } } }); await Promise.all(workers); return results; }
...
const recentCards = recent.map(v =>
    <a class="r-card" href="/search?q=${encodeURIComponent(v.id)}">       <div class="thumb-wrap">         <img src="${escapeHtml(v.thumb)}" alt="${escapeHtml(v.name)}">       </div>       <div class="r-meta">         <div class="r-title">${escapeHtml(v.name)}</div>         <div class="r-sub">ID: ${v.id} • ${fmtDate(v.created_at)}</div>       </div>     </a>  
).join('');
...
; const cards = videos.map(v =>
       <div class="vcard">         <iframe src="https://players.brightcove.net/${AID}/${playerId}_default/index.html?videoId=${v.id}" allow="autoplay; encrypted-media" allowfullscreen></iframe>         <div class="meta">           <div class="title">${escapeHtml(v.name)}</div>           <div class="id">ID: ${v.id} • ${fmtDate(v.created_at)}</div>         </div>       </div>
).join(''); res.send(`<!doctype html>

<html> <head> <meta charset="utf-8"> <title>Results</title> <link rel="preconnect" href="https://fonts.googleapis.com"> <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin> <link href="https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;600;700&display=swap" rel="stylesheet"> <style> body{font-family:'Open Sans',sans-serif;margin:0;padding:20px;} .grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px;} .vcard iframe{width:100%;aspect-ratio:16/9;border:0;} .meta{padding:8px;} .title{font-weight:700;} .id{font-size:12px;color:#6b7280;} a{color:#001f3f;text-decoration:none;font-weight:700;} a:hover{text-decoration:underline;} </style> </head> <body> <a href="/">← Back</a> | <a href="${downloadUrl}">Download Spreadsheet</a> <div class="grid">${cards || '<p>No videos found.</p>'}</div> </body> </html>`); } catch (err) { console.error('Search error:', err.message); res.status(500).send('Error searching.'); } });
// ---------------- Download Spreadsheet ---------------- app.get('/download', async (req, res) => { const qInput = (req.query.q || '').trim(); if (!qInput) return res.status(400).send('Missing search terms'); try { const token = await getAccessToken(); const videos = await unifiedSearch(qInput, token); if (!videos.length) return res.status(404).send('No videos found.');
...
res.end();
} catch (err) { console.error('Spreadsheet error:', err.message); res.status(500).send('Error generating spreadsheet.'); } });
