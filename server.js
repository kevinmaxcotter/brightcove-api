...
// (Note: despite the name, this function escapes characters for HTML)
...
<link relonts.googleapis.comhttps://fonts.gstatic.com
<link href="https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;600=swap
<link rel="preload" as="image" href="${LOGO_LIGHT { --bg:#ffffff; --text:#001f3f; --border:#e5e7eb; --muted:#6b7280; --chip:#eef2f7; --chipBorder:#c7ccd3; --btn:#001f3f; --btnHover:#003366; --btnText:#fff; }
 ...
 *{box-sizing:border-box}
 ...
 /* Show/hide the correct logo based on theme */
...
app.get('/search', async (req, res) => {
  ...
  try {
    ...
    const cards = videos.map(v => `
      <div class="vcard">
        <iframe src="https://players.brightcove.net/${AID}/${PLAYER_ID}_default/index.html?videoId=${v
        <div class="meta">
...
app.get('/download', async (req, res) => {
  ...
  try {
      ...
      while (Date.now() < dlDeadline && idxA < videos.length) {
        ...
        try {
          ...
          try { rows[i] = await getAnalyticsForVideo(v.id, token, range); }
            ...
            rows[i] = { id: v.id, title: v.name || 'Error', tags: v.tags || [],
              views:'N/A', dailyAvgViews:'N/A', impressions:'N/A', engagement:'N/A', playRate:'N/A', secondsViewed:'N/A' };
          }
        }
      }
    }
    ...
    for (let i=0;i<videos.length;i++) {
      if (!rows[i]) {
        const v = videos[i];
        rows[i] = { id: v.id, title: v.name || 'Timeout', tags: v.tags || [],
          views:'N/A', dailyAvgViews:'N/A', impressions:'N/A', engagement:'N/A', playRate:'N/A', secondsViewed:'N/A' };
      }
    }
    ...
    const titleRow = ws.addRow({ id:'Window', title: (range.mode==='range' ? `${range.from} → ${range.to}` : 'All time') });
    ...
    for (const arr of sourcesMap.values()) {
      for (const s of arr) {
        try {
          const u = new URL(s.url); const host = u.host.toLowerCase();
    ...
    let chartABuf = null;
    try { chartABuf = await renderBarChartPNG({ title: 'Top 20 Videos by Views', labels: labelsA, values: dataA, width: 1400, height: 800 }); }
    ...
    let chartBBuf = null;
    try { chartBBuf = await renderBarChartPNG({ title: 'Top 10 Domains by Views', labels: labelsB, values: dataB, width: 1200, height: 700 }); }
