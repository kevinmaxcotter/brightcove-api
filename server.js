...
const stripHtml = s => String(s).replace(/[&<>\"']/g, m => ({
  '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;'
}[m]));
...
app.get('/', async (req, res) => {
  ...
  const recentCards = recent.map(v => `
    /search?q=${encodeURIComponent(v.id)}
      <div class="thumb-wrap">${v.thumb}</div>
      <div class="r-meta">
        <div class="r-title">${stripHtml(v.name)}</div>
...
<body>
  ...
  <main>
    <div class="card">
      ...
      /search
        <input id="q" name="q" placeholder="Examples: 6376653485112, pega platform" required>
